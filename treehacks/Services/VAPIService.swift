...
class VAPIService {
    ...
    /// - Parameters:
    ///   - phoneNumber: The phone number to call (E.164 format, e.g., "+1234567890")
    ///   - contactName: Optional name of the contact for logging
    ///   - completion: Called with success/failure status
    func callPhoneNumber(_ phoneNumber: String, contactName: String? = nil, completion: @escaping (Bool) -> Void) {
        let displayName = contactName ?? phoneNumber
        print("VAPIService: ======== callPhoneNumber() CALLED for \(displayName) ========")
        ...
        let normalizedNumber = phoneNumber.hasPrefix("+") ? phoneNumber : "+\(phoneNumber)"
        ...
        print("VAPIService: === OUTBOUND CALL REQUEST ===")
        print("VAPIService: Calling: \(displayName) at \(normalizedNumber)")
        print("VAPIService: =============================")
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if httpResponse.statusCode == 200 || httpResponse.statusCode == 201 {
                print("VAPIService: ✅ Call to \(displayName) initiated successfully")
                if let data = data, let responseString = String(data: data, encoding: .utf8) {
                    print("VAPIService: Response:\n\(responseString)")
                }
                completion(true)
